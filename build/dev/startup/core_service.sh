...
if [ "$DEBUG_ON" = "1" ]; then
  ...
  -dump_requests \
  -addr :8082 \
  -accepted_jwt_audiences localhost,host.docker.internal,local-dss-core-service,dss_sandbox_local-dss-core-service_1,core-service,1hlc4etfl7fp8tv11r11rclij3 \
  -enable_scd \
  -enable_http
else
  echo "Debug Mode: off"
